...
public:
	...
	UInt64						unk10B8[(0x11C8 - 0x10B8) >> 3];	// 11B8
	BGSLocation*				currentLocation;                    // 11C8
	UInt32						cachedVelocityTimeStamp;			// 11D0
	float						telekinesisDistance;				// 11D4
	float						commandTimer;						// 11D8
	UInt32						unk11DC;							// 11DC
	TESImageSpaceModifier*		unk11E0;							// 11E0
	SInt32						unk11E8;							// 11E8
	UInt32						unk11EC;							// 11EC
	UInt32						unk11F0;							// 11F0
	UInt32						grabType;							// 11F4
	UInt32						unk11F8;
	UInt8						unk11FC;
...
STATIC_ASSERT(offsetof(PlayerCharacter, currentLocation) == 0x11C8);
