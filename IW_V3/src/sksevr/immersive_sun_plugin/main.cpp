...
extern "C" {
	...
	bool SKSEPlugin_Query(const SKSEInterface * skse, PluginInfo * info) {	// Called by SKSE to learn about this plugin and check that it's safe to load it
		gLog.OpenRelative(CSIDL_MYDOCUMENTS, "\\My Games\\Skyrim VR\\SKSE\\ImmersiveSunPluginScript.log");
		gLog.SetPrintLevel(IDebugLog::kLevel_Error);
		gLog.SetLogLevel(IDebugLog::kLevel_DebugMessage);

		std::string logMsg("Immersive Sun V: ");
		logMsg.append(ImmersiveSun::MOD_VERSION);
		_MESSAGE(logMsg.c_str());


		// populate info structure
		info->infoVersion = PluginInfo::kInfoVersion;
		info->name = "ImmersiveSunPluginScript";
		info->version = 010000;
	...
	bool SKSEPlugin_Load(const SKSEInterface * skse) {	// Called by SKSE to load this plugin
		_MESSAGE("ImmersiveSunPluginScript loaded");
		...
		bool btest = g_papyrus->Register(ImmersiveSun::RegisterFuncs);
